...
try {
  ...
  while (await page.locator('button:has-text("Sign in")').count() > 0) {
    ...
    // Watch for the sign-in page (same tab or popup) before we click – Luna uses /login, Amazon uses ap/signin
    const loginPagePromise = Promise.race([
      page.waitForURL(/\/(signin|login)|ap\//i, { timeout: 25000 }).then(() => page),
    ...
    if (email && password) {
      const loginPage = await loginPagePromise.catch(() => page);
      if (loginPage !== page) await loginPage.waitForURL(/\/(signin|login)|ap\//i, { timeout: 15000 }).catch(() => {});
      // Login form can show up as full email+password, as password-only (account already picked), or inside an iframe
      let loginRoot = loginPage;
      const emailSel = '#ap_email, input[name=email][type="email"], input[name=email][type="text"]';
      const passwordSel = '#ap_password, input[name=password]:not([type=hidden])';
      let needEmailStep = true;
      try {
        await loginRoot.locator(emailSel).first().waitFor({ state: 'visible', timeout: 8000 });
      } catch {
        // No email field – could be the “just enter password” screen or the form might be in an iframe
        const passwordLoc = loginPage.locator(passwordSel).first();
        try {
          await passwordLoc.waitFor({ state: 'visible', timeout: 5000 });
          needEmailStep = false; // we’re on the password-only screen
        } catch {
          await loginPage.waitForSelector('iframe', { state: 'attached', timeout: 15000 });
          for (const frame of loginPage.frames()) {
            if (frame === loginPage.mainFrame()) continue;
            try {
              await frame.locator(emailSel).first().waitFor({ state: 'visible', timeout: 5000 });
              loginRoot = frame;
              break;
            } catch { }
          }
          if (loginRoot === loginPage) await loginPage.locator(emailSel).first().waitFor({ state: 'visible', timeout: 15000 }); // give main frame one more shot
        }
      }
      if (needEmailStep) {
        const emailInput = loginRoot.locator(emailSel).first();
        await emailInput.fill(email);
        await loginRoot.locator('input[type="submit"]').first().click();
      }
      // After email submit, Amazon may show: password field (same frame or main page), "security code sent to email", or "account details review".
      // Try password on loginRoot first (same iframe as email), then on loginPage (main frame), to cover both cases.
      let passwordRoot = null;
      try {
        await loginRoot.locator(passwordSel).first().waitFor({ state: 'visible', timeout: 15000 });
        passwordRoot = loginRoot;
      } catch {
        try {
          await loginPage.locator(passwordSel).first().waitFor({ state: 'visible', timeout: 10000 });
          passwordRoot = loginPage;
        } catch {
          let isVerify = false;
          let isReview = false;
          try {
            const url = loginPage.url();
            isVerify = /\/ap\/(cvf|mfa|vf|otp|challenge)/i.test(url)
              || (await loginPage.locator('text=/security code|sent to your email|sent a code|enter the code|one-time|verification code|new device|new browser|new location|verify|we sent/i').count()) > 0
              || (await loginPage.locator('input[name="code"], input[name="otpCode"], #cvf_input_code').count()) > 0;
            isReview = (await loginPage.locator('text=/account details review|review your account|continue|trust this device/i').count()) > 0
              || (await loginPage.locator('input[type="submit"][value="Yes"], input[type="submit"][value="Continue"], button:has-text("Continue"), button:has-text("Yes"), a:has-text("Continue")').count()) > 0;
          ...
          if (isVerify) {
            ...
            const reviewBtn = loginPage.locator('input[type="submit"][value="Yes"], input[type="submit"][value="Continue"], button:has-text("Continue"), button:has-text("Yes"), a:has-text("Continue")');
            if (await reviewBtn.count() > 0) await reviewBtn.first().click().catch(() => {});
            await page.waitForURL(`${BASE_URL}/claims/home?signedIn=true`, { timeout: cfg.login_timeout });
          } else {
            console.log('Unexpected step after email (e.g. password, captcha, or verification). Please complete login in the browser; the script will continue when you reach Prime Gaming.');
            await page.waitForURL(`${BASE_URL}/claims/home?signedIn=true`, { timeout: cfg.login_timeout });
          }
        }
      }
      if (passwordRoot) {
        await passwordRoot.locator(passwordSel).first().fill(password);
        await passwordRoot.locator('input[type="submit"]').first().click();
      }
      loginPage.waitForURL('**/ap/signin**').then(async () => {
        ...
        const error = (await alertLoc.first().innerText().catch(() => '')).trim();
        if (!error) return;
        console.error('Login error:', error);
        await notify(`prime-gaming: login: ${error}`);
        await context.close(); // finishes potential recording
        process.exit(1);
      }).catch(() => {}); // ignore when context/page closed at script end
      // 2FA – fire and forget so we don’t block the main flow
      loginPage.waitForURL('**/ap/mfa**').then(async () => {
        console.log('Two-Step Verification - enter the One Time Password (OTP), e.g. generated by your Authenticator App');
        await loginRoot.locator('[name=rememberDevice]').check();
        ...
        await loginRoot.locator('input[name=otpCode]').pressSequentially(otp.toString());
        await loginRoot.locator('input[type="submit"]').first().click();
      }).catch(_ => { });
    } else {
  ...
  // waitForSelector + textContent avoid patchright "Can't query n-th element" (locator.waitFor/innerText can throw)
  await page.waitForSelector('[data-a-target="user-dropdown-first-name-text"]', { state: 'visible', timeout: cfg.timeout });
  user = (await page.textContent('[data-a-target="user-dropdown-first-name-text"]'))?.trim() ?? '';
